...
import { getJobSeekerScheduledInterview } from "@/data-access/admin-access/analytics/getAnalyticsData";
import { JobSeekerInterview } from "@/lib/prisma-types/Analytics";

interface ScheduledInterviewProps {
  jobseekerId: string;
}

const ScheduledInterview = ({ jobseekerId }: ScheduledInterviewProps) => {
  return (
    <Card>
      <CardHeader className="pb-3">
        <Suspense fallback={<CardTitle>Scheduled Interviews</CardTitle>}>
          <InterviewCountTitle jobseekerId={jobseekerId} />
        </Suspense>
      </CardHeader>
      <CardContent>
        <Suspense fallback={<ScheduleInterviewSkeleton />}>
          <ScheduledInterviewDataFetcher jobseekerId={jobseekerId} />
        </Suspense>
      </CardContent>
    </Card>
  );
...
const InterviewCountTitle = async ({
  jobseekerId,
}: {
  jobseekerId: string;
}) => {
  try {
    const interviews = await getJobSeekerScheduledInterview(jobseekerId);
...
const ScheduledInterviewDataFetcher = async ({
  jobseekerId,
}: {
  jobseekerId: string;
}) => {
  try {
    ...
    return (
      <ErrorDisplay message="There was an error loading the scheduled interviews data" />
    );
  }
};
