...
import ApplicationTableCard from "../TableCard/ApplicationTableCard";
    ...
    <div>
      <div className="items-center py-4 absolute right-0 -top-4 hidden lg:flex">
        <Input
          ...
          className="py-6 w-[150px] lg:w-[250px] place-"
        />
      </div>
      <div>
        ...
        <div>
          <div className="lg:hidden grid grid-cols-1 gap-4  lg:grid-cols-2 xl:grid-cols-3 3xl:grid-cols-4">
            {filteredData.map((data) => {
              const jobData = data as JobApplication;
              return <ApplicationTableCard key={jobData.id} data={jobData} />;
            })}
          </div>
        </div>
        {showPagination && (
